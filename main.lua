    ...
    if plugin_enabled then
        ...
        if teleporting then
            ...
            if not is_loading_screen() then
                ...
                if world_instance then
                    local zone_name = world_instance:get_current_zone_name()
                    if zone_name == waypoint_loader.helltide_tps[current_city_index].name then
                        waypoint_loader.load_waypoints(waypoint_loader.helltide_tps[current_city_index].file)
                        ni = 1
                        teleporting = false
                    elseif os.clock() > next_teleport_attempt_time then
                        console.print("Teleporting...")
                        teleport_to_waypoint(waypoint_loader.helltide_tps[current_city_index].id)
            ...
            if local_player then
                ...
                if current_in_helltide then
                    ...
                    current_city_index = (current_city_index % #waypoint_loader.helltide_tps) + 1
